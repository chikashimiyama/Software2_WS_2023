...
This means 
- the higher the FFT size becomes, the higher resolution we get in the spectrum data
...
Actually, the analysis produced by is two parallel signals. That's because the math involved produces complex numbers, which have a **real** and **imaginary** component. The real components are output from the left outlet and the imaginary components from the middle. Both signals are required for spectral processing.
